...
struct RefreshScrollView<Content: View>: View {
    ...
    @State var lastIsRefreshing = false
    ...
    init(_ threshold: CGFloat = 70, offDown: CGFloat, listH: CGFloat, refreshing: Binding<Bool>, isMore: Binding<Bool>, refreshTrigger: @escaping () -> Void, moreTrigger: @escaping () -> Void, @ViewBuilder content: () -> Content) {
        ...
        self.listH = listH
        self.lastIsRefreshing = refreshing.wrappedValue
    }
    ...
    func calculate(_ values: [RefreshPreferenceTypes.RefreshPreferenceData]) {
        DispatchQueue.main.async {
            ...
            // 刷新变更后更新时间戳
            if (self.isRefreshing != self.lastIsRefreshing) {
                DispatchQueue.main.async {
                    if !isRefreshing {
                        self.updateTime = Date()
                        self.lastIsRefreshing = false
                    }
                }
            }
            ...
            GeometryReader { proxy in
                Color
                    ...
                    .preference(key: RefreshPreferenceTypes.RefreshPreferenceKey.self,
                                value: [RefreshPreferenceTypes.RefreshPreferenceData(viewType: .fixedPositionView,
        ...
        var body: some View {
            GeometryReader { proxy in
                Color
                    .clear
                    .preference(key: RefreshPreferenceTypes.RefreshPreferenceKey.self,
                                value: [RefreshPreferenceTypes.RefreshPreferenceData(viewType: .movingPositionView,
                                                                                     bounds: proxy.frame(in: .global))])
            }
            .frame(height: 0)
        }
